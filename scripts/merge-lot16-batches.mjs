...
const batches = ["A", "B", "C", "D"].map((n) =>
  JSON.parse(readFileSync(resolve(root, `prisma/seed-data/_lot16-batch${n}.json`), "utf8")),
);
...
writeFileSync(
  resolve(root, "prisma/seed-data/dico-btp-lot-16.json"),
  `${JSON.stringify(merged, null, 2)}\n`,
  "utf8",
);
console.log(
  `Fusion Lot 16 OK : ${merged.length} termes (A:${batches[0].length} B:${batches[1].length} C:${batches[2].length} D:${batches[3].length})`,
);
