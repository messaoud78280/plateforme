...
import { adminResendTrialAccessEmail } from "@/lib/platform-admin/actions";
import { publicAppOriginForEmails } from "@/lib/site";

/** POST — Renvoyer l’email d’accès (essai / compte validé). */
export async function POST(request: Request) {
  ...
  const result = await adminResendTrialAccessEmail({
