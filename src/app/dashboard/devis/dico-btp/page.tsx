...
import { BtpDicoFiltersPanel } from "@/components/devis/dico/BtpDicoFiltersPanel";
...
export default async function DicoBtpPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const hasFilters = Boolean(
    sp.q || sp.lot || sp.category || sp.level || sp.status || sp.acronyms || sp.letter || sp.family,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Termes au total" value={stats.totalTerms} />
        <StatCard label="Lots couverts" value={`${stats.lotsCovered} / 16`} />
        <StatCard label="Acronymes" value={stats.acronyms} />
        <StatCard label="Termes à vérifier" value={stats.toVerify} accent={stats.toVerify ? "amber" : undefined} />
      </div>
      ...
      {loadError ? (
        ...
        <div className="space-y-6">
          <p className="text-xs text-slate-500">
            {rows.length} terme{rows.length > 1 ? "s" : ""} affiché{rows.length > 1 ? "s" : ""}
                ...
                {groups.get(letter)!.map((t) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="font-heading text-base font-bold text-slate-900 group-hover:text-[#1e3a5f]">
                          {t.term}
                        </h3>
                        {t.acronym ? <p className="text-xs text-slate-500">{t.acronym}</p> : null}
                      </div>
      ...
      <div id="import-json" className="scroll-mt-24">
...
function StatCard({ label, value, accent }: { label: string; value: string | number; accent?: "amber" }) {
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
      ...
      <p className={`mt-1 text-2xl font-bold ${accent === "amber" ? "text-amber-700" : "text-slate-900"}`}>{value}</p>
    </div>
  );
}
