...
const CHAIN = ["Plan", "Métré", "Devis", "Planning", "Suivi"] as const;
...
export function ProjectPreparationOverview({
  ...
  return (
    <section
      className={cn(
        "rounded-2xl border border-[#1e3a5f]/20 bg-gradient-to-b from-[rgba(30,58,95,0.04)] to-white",
        "px-4 py-5 sm:px-5 sm:py-6",
        "shadow-[0_1px_0_rgba(30,58,95,0.06)]",
      )}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[12.5px] text-slate-600">
            Chaîne principale · plans · métrés · devis · planning · suivi
          </p>
          <p
            className="mt-2.5 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#1e3a5f]/75"
            aria-label="Parcours Plan, Métré, Devis, Planning, Suivi"
          >
            {CHAIN.map((step, i) => (
              <span key={step} className="inline-flex items-center gap-1.5">
                {i > 0 ? (
                  <span className="text-[#1e3a5f]/35" aria-hidden>
                    →
                  </span>
                ) : null}
                <span>{step}</span>
      ...
      0 ? (
        <p className="mt-4 rounded-xl border border-slate-200 bg-white/80 px-3 py-2 text-[12px] text-slate-600">
...
function ScopeBlock({ scope }: { scope: ScopeWorkspace }) {
  return (
    <div className="rounded-2xl border border-[#1e3a5f]/12 bg-white p-3 sm:p-4">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        {scope.cards.map((card) => {
          ...
          return (
            <div
              key={card.kind}
              className="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 p-3 opacity-80"
            >
              {inner}
            </div>
          );
