...
import { CCTP_NORM_REFERENCE_OPTIONS } from "@/content/cctp-norm-references";
...
export function SkillCctpNormPicker({ selected, onChange }: Props) {
  ...
  return (
    <fieldset className="space-y-3 rounded-xl border border-slate-100 bg-slate-50/50 p-4">
      <legend className="px-1 text-sm font-semibold text-slate-800">Références DTU / normes (familles)</legend>
      <p className="text-xs text-slate-500">
        Sélectionnez les familles à prendre en compte. Aucun numéro de norme n&apos;est inventé automatiquement.
      </p>
      <ul className="grid gap-2 sm:grid-cols-2">
        {CCTP_NORM_REFERENCE_OPTIONS.map((opt) => {
