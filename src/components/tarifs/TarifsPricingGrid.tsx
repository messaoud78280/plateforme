...
function OfferCta({ offer }: { offer: BeWorkPublicOffer }) {
  const className =
    offer.recommended
      ? "mt-6 block w-full rounded-lg bg-[#1d4ed8] py-3 text-center text-sm font-semibold text-white shadow-md shadow-[#1d4ed8]/20 transition hover:bg-[#1e40af]"
      : "mt-6 block w-full rounded-lg border-2 border-slate-200 bg-white py-3 text-center text-sm font-semibold text-[#0f172a] transition hover:border-[#1d4ed8]/40 hover:bg-[#f8fafc]";
      ...
      {BEWORK_PUBLIC_OFFERS.map((offer) => {
        ...
        return (
          <article
            ...
            className={`relative flex flex-col rounded-2xl border-2 bg-white p-6 shadow-sm transition hover:shadow-md ${
              featured
          ...
          >
            {featured ? (
              <span className="absolute -top-3 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-full bg-[#1d4ed8] px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow-md">
                Offre recommandée
              </span>
            ) : null}

            <h3 className="text-lg font-bold tracking-tight text-[#0f172a] md:text-xl">{offer.name}</h3>

            <p className="mt-3 text-2xl font-bold leading-tight text-[#1d4ed8] md:text-[1.65rem]">
              {formatOfferPriceLabel(offer)}
            </p>

            <p className="mt-3 text-sm leading-relaxed text-slate-600">{offer.positioning}</p>

            <p className="mt-4 border-t border-slate-100 pt-4 text-sm font-semibold leading-snug text-[#0f172a]">
              {offer.tagline}
            </p>

            {offer.examples && offer.examples.length > 0 ? (
              <div className="mt-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#1d4ed8]">Exemples de missions</p>
                <ul className="mt-2 space-y-1.5 text-sm text-slate-600" role="list">
