...
export async function analyzeMissionFromText(text: string): Promise<MissionSuggestion> {
  ...
  let description =
    "Vous souhaitez confier cette mission à votre assistant. Précisez les éléments importants (clients concernés, délais, documents à utiliser) avant d'envoyer si besoin.";
  let category: MissionSuggestion["category"] = "Autre";
  let priority: MissionSuggestion["priority"] = "STANDARD";
  let estimatedActions: MissionSuggestion["estimatedActions"] = "1 crédit";
  ...
  if (contains("devis")) {
    ...
    estimatedActions = contains("plusieurs", "série", "plusieurs devis") ? "2 à 3 crédits" : "1 crédit";
    ...
    estimatedActions = contains("plusieurs", "liste") ? "2 à 3 crédits" : "1 crédit";
    ...
    category = "Documents";
    estimatedActions = contains("beaucoup", "grand nombre", "dossier complet") ? "2 à 3 crédits" : "1 crédit";
