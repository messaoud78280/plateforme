/**
 * W3-C2A — Exécution manuelle des rappels / escalades (pas de cron).
 */
...
/**
 * Relance manuelle W3-C2A.
 * 1) sync W3-C1 (INITIAL)
 * 2) pour chaque attention active : evaluateAttentionEscalation → REMIND / ESCALATE
 */
export async function processAttentionEscalations(opts?: {
  ...
  await syncAttentionNotificationsForSheets(
    sheets.map((s) => ({
      id: s.id,
      title: s.title,
      status: s.status,
  ...
  for (const [ownerUserId, ownerSheets] of byOwner) {
    ...
    for (const sheet of ownerSheets) {
      ...
      // Filtrer côté JS pour éviter faux positifs contains
      ...
      if (plan.action === "NONE" || !plan.recipientId || !plan.dedupeKey || !plan.notificationType) {
        result.unchanged += 1;
        continue;
      }

      // Garde-fou tenant : destinataire doit être interne et (si org) membre
      const recipient = await prisma.user.findUnique({
        where: { id: plan.recipientId },
        select: { id: true, personType: true },
      });
      if (!recipient || isExternalPerson(recipient.personType) || !isInternalPerson(recipient.personType)) {
        result.skipped += 1;
        continue;
      }
      if (sheet.organizationId) {
        const membership = await prisma.organizationMember.findFirst({
          where: { organizationId: sheet.organizationId, userId: plan.recipientId },
          select: { id: true },
        });
      ...
      try {
        ...
        const code =
          e && typeof e === "object" && "code" in e
            ...
            : "";
        if (code === "P2002") result.unchanged += 1;
        else {
          console.error("[processAttentionEscalations]", e);
          result.skipped += 1;
        }
      }
    }
